...
def cli():
    ...
    parser.add_argument('--detapp',\
            help="Use the detection aperture",\
            action='store_true')
    parser.add_argument('--norebal',\
            help="Don't rebalance image",\
            action='store_true')
...
def main(args):
    ...
    pc_fname = do_pc(dmask, att_fname, work_dir,\
                    ovrsmp=args.oversamp, detapp=args.detapp)
    ...
    mk_sig_imgs_mp(args.Nproc, tbins0, dts, ev_fname, dpi_bkg_fname,\
                pc_fname, att_fname, dmask, work_dir,\
                oversamp=args.oversamp, detapp=args.detapp,\
                rebal=rebal)
