...
def get_simtab_fnames(direc='.'):

    fnames = [fname for fname in os.listdir(direc) if 'sim_tab_' in fname]
    sim_ids = [int((fname.split('_')[2]).split('.')[0]) for fname in fnames]
    return fnames, sim_ids



def main(args):

    log_fname = os.path.join(args.sim_dir, 'rates_llh_analysis_%d.log'%(args.job_id))
    ...
    sim_tab_fnames, simids = get_simtab_fnames(direc=args.sim_dir)

    for sim_tab_fname, sim_id in zip(sim_tab_fnames, simids):

        sim_tab = Table.read(os.path.join(args.sim_dir, sim_tab_fname))
        evdata_ = evdata.copy()
        ev_data = vstack([evdata_, sim_tab])
        ev_data.sort('TIME')
        ...
        save_fname = os.path.join(args.sim_dir, 'rates_llh_res_%d_%d.csv' %(args.job_id, sim_id))
