...
# if a 3rd arg is given, that's used as the min time bin used
...
if [ -f "filter_evdata.fits" ]; then
    ...
    python $batml_path'do_manage2.py' --GWname $gwname --rhel7 --do_bkg --do_rates --do_llh --queue open --pbs_fname $pbsfname --pbs_rhel7_fname $pbsfname > manager.out 2>&1 &
fi
