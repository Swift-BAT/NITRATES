...
def get_intersection_pnt(wall_bnds, gam_theta, gam_phi, batx, baty, batz):
    ...
    denom = (wall_norm_vec[0]*np.sin(gam_theta)*np.cos(gam_phi) +\
            wall_norm_vec[1]*np.sin(gam_theta)*np.sin(-gam_phi) +\
            wall_norm_vec[2]*np.cos(gam_theta))
    if denom == 0.0:
        return np.nan, np.nan, np.nan
    r = -(wall_norm_vec[0]*(batx - wall_x0) + wall_norm_vec[1]*(baty - wall_y0) +\
          wall_norm_vec[2]*(batz - wall_z0))/(wall_norm_vec[0]*np.sin(gam_theta)*np.cos(gam_phi) +\
          wall_norm_vec[1]*np.sin(gam_theta)*np.sin(-gam_phi) + wall_norm_vec[2]*np.cos(gam_theta))
...
def rot_pnts(xs, ys, zs, rot_mat):
    ...
    for i in range(len(x_news)):
        ...
        x_news[i] = xs[i]*rot_mat[0,0] + ys[i]*rot_mat[0,1] + zs[i]*rot_mat[0,2]
        y_news[i] = xs[i]*rot_mat[1,0] + ys[i]*rot_mat[1,1] + zs[i]*rot_mat[1,2]
        z_news[i] = xs[i]*rot_mat[2,0] + ys[i]*rot_mat[2,1] + zs[i]*rot_mat[2,2]
...
def get_intersection_pnts(wall_bnds, gam_theta, gam_phi, batxs, batys, batzs):
    ...
    for i in range(Npnts):
        ...
        if denom == 0.0:
            ...
            continue
        r = -(wall_norm_vec[0]*(batxs[i] - wall_x0) + wall_norm_vec[1]*(batys[i] - wall_y0) +\
              wall_norm_vec[2]*(batzs[i] - wall_z0))/denom
...
class shield_polygon(object):
    ...
    def __init__(self, orig_verts, trans_vec, rotations):
        ...
        if self.Nrots > 0:
            self.verts = [np.matmul(self.rot_mat, vert) + self.trans_vec\
...
class Shield_Structure(object):
    ...
    def __init__(self):
        ...
        x0_hlength = 81.915
        x1_hlength = 81.915
        orig_verts = [np.array([x1_hlength, y_hlength, 0.0]),
        ...
        orig_verts = [np.array([x1_hlength, y_hlength, 0.0]),
                      ...
                      np.array([-x0_hlength, -y_hlength, 0.0]),
                      np.array([x0_hlength, -y_hlength, 0.0])]
        rot_dicts_secB = [{'axis':'X', 'angle':np.radians(20.36)},
                           {'axis':'Y', 'angle':np.radians(-90.0)},
                           {'axis':'X', 'angle':np.radians(-90.0)}]
        rot_dicts_secBFlip = [{'axis':'X', 'angle':np.radians(-20.36)},
                               {'axis':'Y', 'angle':np.radians(-90.0)},
                               {'axis':'X', 'angle':np.radians(-90.0)}]
        x_mid = 88.405
        y_mid = 7.623
        ...
        orig_verts = [np.array([x1_hlength+x_offset, y_hlength, 0.0]),
...
#                       np.array([-x0_hlength-x_offset, -y_hlength, 0.0]),
    ...
    #     x0_hlength = 81.915
        ...
        rot_dicts_secD = [{'axis':'X', 'angle':np.radians(-2.385)},
                          {'axis':'Y', 'angle':np.radians(-45.0)},
                          {'axis':'X', 'angle':np.radians(90.0)}]

        rot_dicts_secDflip = [{'axis':'X', 'angle':np.radians(2.385)},
...
class Shield_Interactions(object):
    ...
    def which_poly_it_intersects(self, theta, phi, x, y, z, polyIDs2ignore=[]):
        ...
        for ident in range(self.Npolys):
            ...
            does_int[no_ints_yet] = self.does_intersect_poly(theta, phi, x[no_ints_yet],\
                                                y[no_ints_yet], z[no_ints_yet],\
                                                ident)
    ...
    def get_angs2norm(self, theta, phi):
        ...
        gamma_vec = np.array([np.sin(theta)*np.cos(phi),
...
class Sun_Shield_Structure(object):
    ...
    def __init__(self):
        ...
        rot_dicts2 = [{'axis':'Y', 'angle':np.radians(-55.0)},
                      {'axis':'X', 'angle':np.radians(90.0)}]
        ...
        rot_dicts3 = [{'axis':'Y', 'angle':np.radians(-125.0)},
                      {'axis':'X', 'angle':np.radians(90.0)}]
