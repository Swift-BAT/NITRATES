...
from rates_only_quads_wlin_mle import get_quad_cnts_tbins, get_cnts_per_tbins
...
def cubic_err(cov_mat, t_ax):
    ...
    sigs2 = np.diag(cov_mat)
    err = np.hypot(sigs2[0], sigs2[1]*t_ax, sigs2[2]*(t_ax**2), sigs2[3]*(t_ax**3))
...
def get_cub_rate_quad_objs(quad_dicts, ev_data, trig_time, ebins0, ebins1,\
    ...
    t_bins0 = np.arange(-15.*1.024, 15.*1.024, tstep) + trig_time
...
def get_lin_rate_quad_objs(quad_dicts, ev_data, trig_time, ebins0, ebins1,\
    ...
    for direc, quad_dict in quad_dicts.iteritems():
        ...
        lin_rate_quad_dict[direc] = Linear_Rates(cnts_per_tbin, t_bins0, t_bins1, trig_time)
...
def get_lin_and_cub_rate_quad_obs(quad_dict, ev_data, trig_time,\
...
def get_lin_rate_obj(ev_data, trig_time, ebins0, ebins1,\
                        bin_size=.512, tstep=.512, trng=15):
...
class Cubic_Rates(object):
    ...
    def __init__(self, cnts_per_tbin, t_bins0, t_bins1,\
        ...
        self.t0 = trig_time - 15.*self.t_poly_step
        self.t1 = trig_time + 15.*self.t_poly_step
