...
from funcs2run_bat_tools import do_bkg, std_grb, do_pc
...
def cli():
    ...
    parser.add_argument('--att_fname', type=str,\
            ...
            default="attitude.fits")
    parser.add_argument('--tstart', type=float,\
            help="Time to start first exp at in MET")
...
def main(args):
    ...
    tstarts = np.arange(start_time, start_time + args.search_twind,\
                        args.tstep)
    ...
    for i in range(Nexps2do):
        ...
        bkg_dpi = [do_bkg(bkg_tstarts[i], bkg_tends[i],\
                    args.evfname, args.dmask, '.', e0=args.e0, e1=args.e1)]

        std_grb(tstarts[i], args.exp, args.evfname, bkg_dpi, args.att_fname,\
                args.dmask, '.', pc=pcfname, e0=args.e0, e1=args.e1,\
