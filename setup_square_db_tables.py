...
def setup_job_tables(Njobs, conn, pc_fname, im_steps=10,\
                    pc_min=0.1, sig_pix_tab=None, do_stat=False):
    ...
    bins = [np.linspace(-2,2,im_steps*4+1),
            np.linspace(-1,1,im_steps*2+1)]
...
def cli():
    ...
    parser.add_argument('--Njobs', type=int,\
            help="Number of jobs to run for LLH analysis",\
            default=24)
    ...
    parser.add_argument('--pcfname', type=str,\
            ...
            default=None)
    parser.add_argument('--dostat',\
            help="Setup status table",\
            action='store_true')
    parser.add_argument('--imsteps', type=int,\
            help="Steps per 1 imx/y",\
            default=10)
...
def main(args):
    ...
    if args.pcfname is None:
        ...
        try:
            ...
            fnames = os.listdir('.')
            pc_fnames = [fname for fname in fnames if\
                    ('img' in fname) and (('pc' in fname) or ('PC' in fname))]
            if len(pc_fnames) < 1:
    ...
    setup_job_tables(args.Njobs, conn, pc_fname, do_stat=args.dostat,\
                    im_steps=args.imsteps)
