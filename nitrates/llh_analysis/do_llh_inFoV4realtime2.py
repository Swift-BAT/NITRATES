...
def cli():
    ...
    parser.add_argument("--keep_all", help="Keep all results", action="store_true")
    parser.add_argument("--TS2keep", type=float, help="Min TS to write", default=4.5)
    parser.add_argument(
        "--minTS2scan",
        type=float,
...
def main(args):
    ...
    do_analysis(
        ...
        args.bkg_fname,
        args.keep_all,
        TS2keep=args.TS2keep,
        minTS2scan=args.minTS2scan,
    )
    ...
    for ind, row in shuffled_square_tab.iterrows():
        ...
        logging.info(
            "squareID: %d no results yet, starting analysis" % (row["squareID"])
        )

        do_analysis(
            row["proc_group"],
            shuffled_square_tab[bl],
            ev_data,
            flux_mod,
            rt_dir,
            ebins0,
