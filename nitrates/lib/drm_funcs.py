...
import os
from logllh_ebins_funcs import get_cnt_ebins_normed, get_cnt_ebins

def get_drm_arr(drm_dir):

    drm_fnames = np.array([fn for fn in os.listdir(drm_dir) if\
                    'drm_' in fn])
...
class DRMs(object):
    ...
    def get_closest_ind(self, imx, imy):
        ...
        return np.argmin(np.hypot(imx-self.drm_arr['imx'],\
                                  imy-self.drm_arr['imy']))
...
class cnts_intp(object):
    ...
    def __call__(self, ind):
        ...
        cnts = A0*self.cnt_ebins_ind_mat[ind_ind0] +\
