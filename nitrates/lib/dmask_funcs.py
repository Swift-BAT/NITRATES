...
def get_hotpix_map(ev_data, bl_dmask, mask_zeros=True):
    ...
    if mask_zeros and (np.median(cnts)>20):
        cnts = cnts[(cnts>0)]
    perc01, perc99 = np.percentile(cnts, [1., 99.])
    # lower_limit = perc01 - .25*np.median(cnts) - 1.
    lower_limit = stats.poisson.ppf(.1/ndets, 0.85*np.median(cnts)) - 1.
    if mask_zeros and lower_limit < 1:
        lower_limit = 0.5
    upper_limit = perc99 + .25*np.median(cnts) + 5.
...
def find_rate_spike_dets2mask(evdata, GTI=None, emax=50, tbin_size=20e-3, max_cnts=10):
    ...
    detx_bins = np.arange(np.max(evdata['DETX'])+1) - .5
    dety_bins = np.arange(np.max(evdata['DETY'])+1) - .5
    ...
    for i in range(tsteps):
        t1 = t0 + tstep
        tbins = np.arange(t0, t1+tbin_size/2., tbin_size)
