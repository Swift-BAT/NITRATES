...
class Norm_3D(object):
    ...
    def logpdf(self, x_, y_, z_):
        ...
        exp_term = ((self.rhoyz**2)-1.)*(x**2/self.sigx2) + ((self.rhoxz**2)-1.)*(y**2/self.sigy2) +\
                    ((self.rhoxy**2)-1.)*(z**2/self.sigz2) +\
                    2*(self.rhoxy - self.rhoxz*self.rhoyz)*(x*y/(self.sigx*self.sigy)) +\
                    2*(self.rhoxz - self.rhoxy*self.rhoyz)*(x*z/(self.sigx*self.sigz)) +\
                    2*(self.rhoyz - self.rhoxy*self.rhoxz)*(y*z/(self.sigy*self.sigz))

        self._log_pdf = -.5*( self._logpdf_x0 + self.inv_cov_coef*exp_term)
...
class Norm_2D(object):
    ...
    def set_cov(self, cov):
        ...
        self.pdf_coef = 1./( 2.*np.pi*self.sigx*self.sigy*np.sqrt(1. - self.rho**2) )
        self.pdf_coef_log = np.log(self.pdf_coef)
        self.exp_term_coef = -1./( 2.*(1. - self.rho**2) )
...
#         self._logpdf_x0 = np.log(((2.*np.pi)**3)*self.cov_det)
    ...
    def logpdf(self, x_, y_):
        ...
        exp_term = ((x**2)/self.sigx2) + ((y**2)/self.sigy2) - 2.*self.rho*x*y/(self.sigx*self.sigy)
    ...
    def jacob_log_pdf(self, x_, y_):
        ...
        jacob[0] = 2*x/self.sigx2 - 2.*self.rho*y/(self.sigx*self.sigy)
        jacob[1] = 2*y/self.sigy2 - 2.*self.rho*x/(self.sigx*self.sigy)
    ...
    def set_hess_log_pdf(self):
        ...
        hess[0,0] = self.exp_term_coef*2./(self.sigx2)
        hess[1,1] = self.exp_term_coef*2./(self.sigy2)

        hess[0,1] = -2.*self.exp_term_coef*self.rho/(self.sigx*self.sigy)
        hess[1,0] += hess[0,1]
...
class Norm_1D(object):
    ...
    def set_cov(self, cov):
        ...
        self.pdf_log_term = -np.log(self.sig) - np.log(2.*np.pi)/2.
        self.set_hess_log_pdf()
