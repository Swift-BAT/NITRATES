...
def cli():
    ...
    parser.add_argument('--imy1', type=float,\
            help="imy high value",\
            default=1.0)
    parser.add_argument('--imstep', type=float,\
            help="step size in imx/y",\
            default=0.05)
...
def main(args):
    ...
    imxs = np.linspace(-2., 2., 40*4+1)[1:-1]
    ...
    on_off_map = os.path.join(hk_dir, [fname for fname in os.listdir(hk_dir) if 'bdecb' in fname][0])
    ...
    pha_fname = os.path.join(args.workdir, str(pid) + '_pha_%.3f_%.3f_.pha' %(tstart, tstop))
    ...
    for i in range(len(imxs)):
        ...
        drm_fname = os.path.join(args.savedir, 'drm_%.3f_%.3f_.fits' %(imxs[i], imys[i]))
