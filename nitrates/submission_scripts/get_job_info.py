...
def cli():
    ...
    parser.add_argument('--ssh',\
            help="Do we need to ssh in to submit?",\
            action='store_true')
    parser.add_argument('--name', type=str,\
            help="Name of jobs to query",\
            default=None)
...
def get_env_vars(jobid):
    ...
    cmd_list = ['/opt/moab/bin/checkjob', '-v', '-v', str(jobid)]#, '|', 'grep', 'EnvVariables']
    cmd_list2 = ['grep', 'EnvVariables']
    #print cmd_list
    #cmd_list = shlex.split(cmd)
    out1 = subprocess.Popen(cmd_list,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT)
    out = subprocess.Popen(cmd_list2,
            stdin=out1.stdout,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT)
