...
def do_analysis(square_tab, t0s, t1s, pc_imxs, pc_imys,\
                pl_flux, drm_obj, rt_dir,\
                fp_dir,\
                bkg_llh_obj, sig_llh_obj,\
                trigger_time, work_dir):

    bkg_rates = np.array([0.08922625, 0.06397256, 0.0491008,  0.04211757, 0.03229525, 0.02950153])
    bkg_errs = np.array([0.0058777, 0.00497689, 0.00436019, 0.00403825, 0.00353615, 0.00337974])
    bkg_flat_diff_ratios = np.array([5.60687347e-04, 1.75202892e-02, 3.13383457e-01,
                                    5.93355830e-01, 9.25799407e-01, 6.38147129e-01])
...
def main(args):
    ...
    dts = np.array([1.024,
                    ...
                    0.512, 0.256,
                    0.128])
    t0s = np.array([0.0, 0.064]) + trigtime
    dts = np.array([0.128, 0.128])
    t1s = t0s + dts

    fp_dir = '/gpfs/scratch/jjd330/bat_data/footprints_npy/'

    do_analysis(square_tab, t0s, t1s, pc_imxs, pc_imys, pl_flux,\
