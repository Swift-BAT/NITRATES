...
def do_analysis(
    ...
    bkg_rates = np.array(
        [0.08922625, 0.06397256, 0.0491008, 0.04211757, 0.03229525, 0.02950153]
    )
    bkg_errs = np.array(
        [0.0058777, 0.00497689, 0.00436019, 0.00403825, 0.00353615, 0.00337974]
    )
    ...
    for square_ind, square_row in square_tab.iterrows():
        ...
        for tt in range(Ntbins):
            ...
            try:
                logging.debug(
                    "Min NLLH: %.2f" % (np.nanmin(sig_nllhs[(sig_nllhs > 0)]))
                )
            except:
                pass
...
def main(args):
    ...
    t0s = np.array([-10.0, -8.0, -6.5, -0.128, 0.0, 0.128]) + trigtime
    dts = np.array([1.024, 0.256, 0.128, 0.512, 0.256, 0.128])
