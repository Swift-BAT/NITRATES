...
from config import quad_dicts, EBINS0, EBINS1, drm_quad_dir, solid_angle_dpi_fname, fp_dir
...
def gauss_nllh2min_2regs(theta, data_counts, bkg_counts, bkg_err,\
    ...
    nllh0 = gauss_sig_bkg_nllh(data_counts, Nsigs*Ndet_frac, bkg_counts,\
    ...
    nllh1 = gauss_sig_bkg_nllh(data_counts2, Nsigs*(1.-Ndet_frac)*Atrans, bkg_counts2,\
...
def min_det_fp_nllh(data_counts, model, fp_bl,\
    ...
    tot_err2 += bkg_err**2
    for i in range(Nsrcs):
        ps_rate_errs = np.array([params[PSnames[i] + '_rate_' + str(j)]\
...
def min_det_fp_nllh2(data_counts0, data_counts1, model, fp_bl,\
                    params, PSnames, bkg_name,\
                    dt, cnts_intp,\
                    Ndets0, Ndets1, solid_ang_dmean0, solid_ang_dmean1,\
                    rt_sums0, rt_sums1, Atrans,\
                    get_bkg_nllh=False):
    ...
    bkg_flatAs = np.array([params[bkg_name + 'flat_' + str(j)] for j in range(nebins)])
    bkg_diffAs = 1. - bkg_flatAs
    bkg_err0 = bkg_rate_errs*(bkg_flatAs*Ndets0 + bkg_diffAs*solid_ang_dmean0)*dt
    bkg_err1 = bkg_rate_errs*(bkg_flatAs*Ndets1 + bkg_diffAs*solid_ang_dmean1)*dt
...
#     args = (data_counts, bkg_cnts, bkg_err, cnts_intp)
    ...
    args = (data_counts0, bkg_cnts0, bkg_err0, data_counts1,\
            bkg_cnts1, bkg_err1, cnts_intp, Ndet_frac, Atrans)
...
#         bkg_nllh1 = gauss_sig_bkg_nllh(data_counts1, 0., bkg_cnts1, bkg_err1)
        bkg_nllh = gauss_nllh2min_2regs([-10.0, 1.5], *args)
        return bf_nsig, bf_ind, nlogls[best_ind], bkg_nllh
...
def get_cnts_from_tbins_ebins(evdata, tbins0, tbins1, ebins0, ebins1):
    ...
    dts = np.diff(tbins0)
    breaks = np.where(dts>tstep)
    tfreq = int(np.rint(tbin_size/tstep))
    i0 = 0

    for bi in range(len(breaks[0])+1):

        if bi >= len(breaks[0]):
            break_ = len(tbins0)
        else:
            break_ = breaks[0][bi]
        tbins0_ = tbins0[i0:break_+1]
        t_add = [tbins0_[-1] + (i+1)*tstep for i in range(tfreq)]
        tbins = np.append(tbins0_, t_add)
        h = np.histogramdd([evdata['TIME'],evdata['ENERGY']], bins=[tbins,ebins],\
                        range=((tbins[0],tbins[-1]),(ebins[0],ebins[-1])))[0]
...
#         self.cnts_tot = np.zeros((ntbins,self.nebins))

#         h = np.histogramdd([self.ev_data['TIME'],\
#                     self.ev_data['ENERGY']],
#                bins=[tbins,ebins])[0]
...
#             self.cnts_fpte[ii] = h2
    ...
    def run(self):
        ...
        for ii, exp_group in enumerate(self.exp_groups):
            ...
            logging.debug("tbin_size: %.3f" %(tbin_size))
            ...
            for jj in range(self.Nfps):
                ...
                cnts_per_tbin1 = self.cnts_tot - cnts_per_tbin
                Ndets = self.ndets[jj]
                solid_ang = self.solid_angs[jj]
                rt_sums = self.rt_sums[jj]
                rt_sums1 = self.rt_sums1[jj]
                fpbl = self.fpbls[jj]
                imx = self.imxs[jj]
                imy = self.imys[jj]
...
def main(args):
    ...
    imxax = np.arange(-1.6, 1.61, 0.16)
    imyax = np.arange(-.9, .91, 0.05)

    imxg, imyg = np.meshgrid(imxax, imyax)
    imxs = imxg.ravel()
    imys = imyg.ravel()
    bl = np.isclose((imys%.1),0)
    imxs[bl] += .08
    ...
    min_pc = max(args.min_pc - 0.25, 0.0099)
