...
def do_analysis(seed_tab, pl_flux, drm_obj, rt_obj,\
                ...
                conn, db_fname, grid_width=2e-3, grid_step=1e-3):
    ...
    for seed_g in seed_t_gs:
        ...
        for row in seed_tab.itertuples():
            ...
            try:
                ...
                sig_mod = Point_Source_Model(row.imx, row.imy, grid_width,\
                                    pl_flux, drm_obj,\
                                    [ebins0,ebins1], rt_obj, bl_dmask, use_deriv=True)
                ...
                param_list, nllhs, imxs, imys = imxy_grid_miner(\
                            sig_miner, row.imx-grid_width/2., row.imy-grid_width/2.,\
                            row.imx+grid_width/2., row.imy+grid_width/2.,\
                            dimxy=grid_step)
        ...
        try:
            write_results(conn, rows, sig_param_dicts,\
