...
python $batml_path'do_data_setup.py' --work_dir $workdir --trig_time $trigtime --search_twind $twind --min_dt $tmin
if [ -f "filter_evdata.fits" ]; then
    ...
    python $batml_path'do_manage.py' --Nrate_jobs $Nratejobs --GWname $gwname --do_bkg --do_rates --do_llh --do_scan > manager.out 2>&1 &
fi
