...
def cli():
    ...
    parser.add_argument('--Nratejobs', type=int,\
            help="Total number of jobs",
            default=64)
    parser.add_argument('--Nllhjobs', type=int,\
            help="Total number of jobs",
            default=128)
    ...
    parser.add_argument('--do_scan',\
            help="Submit the llh scan jobs",\
            action='store_true')
    parser.add_argument('--do_peaks',\
            help="Submit the peak scan jobs",\
            action='store_true')
...
def main(args):
    ...
    if args.do_bkg:
        sub_jobs(1, 'BKG_'+args.Name, args.BKGpyscript,\
                    args.pbs_fname, queue=args.queue, qos=args.qos, ppn=5)
    if args.do_rates:
        sub_jobs(args.Nratejobs, 'RATES_'+args.Name, args.RATEpyscript,\
                    args.pbs_fname, queue=args.queue, qos=args.qos)
    if args.do_llh:
        sub_jobs(args.Nllhjobs, 'LLH_'+args.Name, args.LLHpyscript,\
                    args.pbs_fname, queue=args.queue, qos=args.qos)
    if args.do_scan:
        sub_jobs(args.Nllhjobs, 'SCAN_'+args.Name, args.SCANpyscript,\
                    args.pbs_fname, queue=args.queue, qos=args.qos)
    if args.do_peaks:
        sub_jobs(args.Nllhjobs, 'PEAK_'+args.Name, args.PEAKpyscript,\
                    args.pbs_fname, queue=args.queue, qos=args.qos)
