...
def cli():
    ...
    parser.add_argument('--savedir', type=str,\
            help="Directory to save tmp files to",
            default="/tmp/")
    ...
    parser.add_argument('--TScut', type=float,\
            help="Rate TS to cut at",\
            default=2.0)
    parser.add_argument('--trigtime', type=float,\
            help="trigger time in MET secs")
...
def main(args):
    ...
    timeIDs, RateTSvals = get_top_rate_timeIDs(conn, TScut=args.TScut,\
                                                ret_TSs=True)
    ...
    mk_sig_imgs_pix(times, dts, ev_fname, dpi_bkg_fname,\
                pc_fname, att_fname, dmask, savedir,\
                work_dir,\
                trig_time, conn, db_fname, oversamp=2,\
                snr_cuts=snr_cuts)
