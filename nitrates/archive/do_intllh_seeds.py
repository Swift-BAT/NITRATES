...
def cli():
    ...
    parser.add_argument('--seed_fname', type=str,\
...
HALF_LOG2PI = .5*np.log(2*np.pi)

def norm_logpdf(x, sig, mu):
    return -np.square((x-mu)/sig)/2. - np.log(sig) - HALF_LOG2PI
...
class Bkg_Model_wSAfixed(Model):
    ...
    def __init__(self, bl_dmask, solid_ang_dpi, nebins,\
                 flat_diff_ratios, exp_rates=None,\
                 bkg_sigs=None, use_prior=False, use_deriv=False):
...
#             rates = bkg_obj.get_rate(t)[0]
        ...
        super(Bkg_Model_wSAfixed, self).__init__('Background', bl_dmask,\
                                        param_names, param_dict,\
...
#         dr_dDifs = np.zeros((self.nebins,self.ndets))
        ...
        for pname in self.param_names:
            ...
            if j != j0:
                continue
            dr_dps.append(self.diff_As[j0]*self.solid_angs/\
                          self.solid_ang_mean + self.flat_As[j0])
...
#                                    scale=self.bkg_sigs[j0])
    ...
    def get_dnlp_dp(self, params, j):
        ...
        for pname in self.param_names:
            ...
            if j != j0:
                continue
            dnlp_dps.append( (params[pname]-self.exp_rates[j0])/\
                            np.square(self.bkg_sigs[j0]) )
...
class LLH_webins(object):
    ...
    def __init__(self, event_data, ebins0, ebins1,\
                 bl_dmask, t0=None, dt=None, model=None):
...
def log_prob2min(frac_bkg_rate, bkg_exp_rate, bkg_err, llh_obj, params, ebin):
    ...
    try:
        ...
        logging.error(E)
        logging.error(traceback.format_exc())
    return nllh + np.log(frac*(np.log(.5) - np.log(1e-3))) -\
            stats.norm.logpdf(bkg_rate, scale=bkg_err, loc=bkg_exp_rate)


def var_around_min(frac_min, bkg_min, bkg_err, llh_obj, params,\
                   ebin, sig_mod, bkg_mod):
...
#     dnLP_df2 = 1./frac_min**2
    ...
    if sig_mod.prior_type == 'log_norm':
        ...
        dnLP_df2 = -(-frac_mu + frac_sig**2 + np.log(frac_min) - 1)/\
                    np.square(frac_sig*frac_min)
    ...
    dnLP_db2 = 1./(bkg_err)**2

    return 1./(dnLLH2_df + dnLP_df2), 1./(dnLLH2_db + dnLP_db2), 1./dnLLH2_dfb


def integrated_LLH_webins(llh_obj, miner, params, bl_dmask,\
                          bkg_rates, bkg_errs, sig_mod, bkg_mod):
...
#         Nfunc_calls += res[3]
        frac_var, bkg_var, fb_cov = var_around_min(bf_frac, bf_bkg_rate,\
                                        bkg_errs[j], llh_obj, params,\
                                        j, sig_mod, bkg_mod)

        I_mat = np.matrix([[1./frac_var, 1./fb_cov],[1./fb_cov, 1./bkg_var]])
...
def int_LLH2min(A, llh_obj, miner, params, sig_mod, bkg_mod, bl_dmask,\
                bkg_rates, bkg_errs):

    params['Signal_A'] = A
    miner.set_fixed_params(['Signal_A'], [A])
    return -integrated_LLH_webins(llh_obj, miner, params, bl_dmask, bkg_rates,\
                                  bkg_errs, sig_mod, bkg_mod)


def int_LLH2min_Agamma(A_gamma, llh_obj, miner, params, sig_mod, bkg_mod,\
                       bl_dmask, bkg_rates, bkg_errs):
...
def do_analysis(seed_tab, pl_flux, drm_obj, rt_dir,\
                fp_dir, bkg_llh_obj, sig_llh_obj,\
                trigger_time, work_dir, jobid):

    bkg_rates = np.array([0.07609196, 0.05611105, 0.04255688, 0.03599852, 0.02635129, 0.02274061])
    bkg_errs = .05*bkg_rates
    bkg_flat_diff_ratios = np.array([0.00065753, 0.00089188, 0.16403959, 0.00139088, 0.00190105, 0.4973408 ])
    ...
    for timeID, seeds in tgrps:
        ...
        if Npix < 1:
            fname = os.path.join(work_dir,\
                    'int_llh_res_%d_%d_.csv' %(timeID, jobid))
            logging.info("Nothing to write for timeID %d"\
                        %(timeID))
            f = open(fname, 'w')
            f.write('NONE')
            f.close()
        ...
        logging.debug("Starting time: %.3f, with duration %.3f"\
                    %(t0, dt))
        ...
        bkg_mod = Bkg_Model_wSAfixed(bl_dmask, solid_ang_dpi,\
                                    nebins, bkg_flat_diff_ratios,\
                                    exp_rates=bkg_rates, bkg_sigs=bkg_errs,\
                                    use_prior=True, use_deriv=True)

        imx_, imy_ = np.nanmean(imxs), np.nanmean(imys)

        sig_mod = Point_Source_Model_Wuncoded(imx_, imy_, 0.3, pl_flux, drm_obj,\
                                 [ebins0,ebins1], rt_obj, fp_obj, bl_dmask,\
        ...
        params_ = {bkg_mod.name+'_'+bkg_mod.param_names[j]:bkg_rates[j]\
                    for j in range(nebins)}
        ...
        for ii in range(Npix):
            ...
            try:
                logging.debug('iter: %d, imx: %.4f, imy: %.4f'%(ii,imxs[ii],imys[ii]))
                res = optimize.minimize(int_LLH2min_Agamma, [.025, .75], method='Nelder-Mead',\
                                        args=(sig_llh_obj, sig_miner, params_,\
                                        sig_mod, bkg_mod, bl_dmask,\
                                        bkg_rates, bkg_errs),\
                                        options={'fatol':1e-2, 'xatol':1e-3,
                                        'maxfev':360})
...
def main(args):
    ...
    do_analysis(seed_tab, pl_flux,\
                    drm_obj, rt_dir, fp_dir,\
                    bkg_llh_obj, sig_llh_obj,\
                    trigtime, work_dir, proc_num)
