...
def main(args):
    ...
    logging.basicConfig(filename='rates_llh_analysis.log', level=logging.DEBUG,\
    ...
    avg_rate_quad_dict, lin_rate_quad_dict = get_quad_rate_objs_from_db(conn, quad_dicts)
    ...
    for i, exp_group in enumerate(exp_groups):
        ...
        quad_cnts_mat = get_quad_cnts_tbins_fast(t_bins0, t_bins1,\
                                        ebins0, ebins1, ev_data)
        ...
        for direction, quad_dict in quad_dicts.items():
            ...
            if args.nproc > 1:
                bkg_llh_tbins, llhs, bf_nsigs, bf_inds =\
                        do_rate_mle_mp(cnts_per_tbin, lin_rate_quad_dict[direction],\
                                    cnts_intp, t_bins0, t_bins1,\
                                    nproc=args.nproc, bkg_err_fact=2.0)
            else:
                bkg_llh_tbins, llhs, bf_nsigs, bf_inds =\
                        do_rate_mle(cnts_per_tbin, lin_rate_quad_dict[direction],\
