...
def do_analysis(seed_tab, pl_flux, drm_obj, rt_obj,\
                ...
                conn, db_fname, trigger_time, TS_min=4.0):
    ...
    for imxy_inds, df_imxys in seed_imxy_groups:
        ...
        for timeID, seed_df in df_twinds:
            ...
            sig_mod = Point_Source_Model(np.mean(seed_df['imx']),\
                                    np.mean(seed_df['imy']), 0.3,\
                                    pl_flux, drm_obj,\
            ...
            for index, row in seed_df.iterrows():
                try:
                    sig_miner.set_fixed_params(['Signal_imx', 'Signal_imy'], [row['imx'],row['imy']])
        ...
        try:
            write_results_fromSigImg(conn, rows, t0s, dts, sig_param_dicts,\
