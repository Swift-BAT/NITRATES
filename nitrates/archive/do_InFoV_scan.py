...
def analysis_for_imxy_square(
    ...
    sig_mod = Source_Model_InFoV(
        flux_mod, [ebins0, ebins1], bl_dmask, rt_obj, fp_obj, use_deriv=True
    )
    ...
    for i in range(ntbins):
        ...
        res_df["bkg_nllh"] = bkg_nllh
        res_df["TS"] = np.sqrt(2.0 * (bkg_nllh - res_df["nllh"]))
...
def main(args):
    ...
    bkg_bf_params = {
        bkg_mod.param_names[i]: pars[0][i] for i in range(len(bkg_mod.param_names))
    }
