...
from ..models.models import Bkg_Model_wSA, Point_Source_Model,\
...
def cli():
    ...
    parser.add_argument('--seeds_fname', type=str,\
...
def do_analysis(square_tab, seed_tab, pc_imxs, pc_imys,\
    ...
    for square_ind, square_row in square_tab.iterrows():
        ...
        if Npix < 1:
            fname = os.path.join(work_dir,\
                    'scan_res_%d_.csv' %(square_row['squareID']))
            ...
            for ii in range(Npix):
                try:
                    ...
                    if not res[0].success:
                        logging.warning("Unsucessful minimize at: imx %.3f imy %.3f"%(imxs[ii],imys[ii]))
...
def main(args):
    ...
    do_analysis(square_tab, seed_tab, pc_imxs, pc_imys, pl_flux,\
                    drm_obj, rt_dir,\
                    bkg_llh_obj, sig_llh_obj,\
                    conn, db_fname, trigtime, work_dir, args.bkg_fname)
