...
def cli():
    ...
    parser.add_argument(
        ...
        default="scan_job_table.csv",
    )
    parser.add_argument(
        "--seeds_fname",
        type=str,
        help="positions and times to scan at file name",
        default="scan_seeds.csv",
    )
    parser.add_argument(
        "--bkg_fname",
        type=str,
    ...
    parser.add_argument(
        "--log_fname", type=str, help="Name for the log file", default="llh_scan"
    )
...
def do_analysis(
    ...
    for square_ind, square_row in square_tab.iterrows():
        ...
        if Npix < 1:
            fname = os.path.join(
                work_dir, "scan_res_%d_.csv" % (square_row["squareID"])
            )
            logging.info("Nothing to write for squareID %d" % (square_row["squareID"]))
            f = open(fname, "w")
            f.write("NONE")
            f.close()
            continue
        ...
        bl = seed_tab["squareID"] == square_row["squareID"]

        logging.info("%d timeIDs to do" % (np.sum(bl)))

        seeds = seed_tab[bl]

        for seed_ind, seed_row in seeds.iterrows():
            logging.debug(
                "Starting timeID: %d, table index: %d" % (seed_row["timeID"], seed_ind)
            )

            res_dict = {}
            res_dict = {
                "squareID": square_row["squareID"],
                "timeID": seed_row["timeID"],
            }
            ...
            for ii in range(Npix):
                try:
                    ...
                    if not res[0].success:
                        logging.warning(
                            "Unsucessful minimize at: imx %.3f imy %.3f"
                            % (imxs[ii], imys[ii])
                        )
        ...
        fname = os.path.join(work_dir, "scan_res_%d_.csv" % (square_row["squareID"]))
...
def main(args):
    ...
    logging.debug(
        "Min pc_imx, pc_imy: %.2f, %.2f" % (np.nanmin(pc_imxs), np.nanmin(pc_imys))
    )
    logging.debug(
        "Max pc_imx, pc_imy: %.2f, %.2f" % (np.nanmax(pc_imxs), np.nanmax(pc_imys))
    )
    ...
    seed_tab = pd.read_csv(args.seeds_fname)

    logging.info("Read in Square and Seeds Tables, now to do analysis")
