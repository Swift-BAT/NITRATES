...
def gauss_nllh2min_2regs(
    ...
    nllh0 = gauss_sig_bkg_nllh(
        data_counts, Nsigs * Ndet_frac, bkg_counts, bkg_err, sys_err=sys_err
    )

    if Ndet_frac > 0.99:
        return nllh0

    sys_err2 = np.sqrt(sys_err**2 + (Atrans / 4.0**2))

    nllh1 = gauss_sig_bkg_nllh(
        data_counts2,
        Nsigs * (1.0 - Ndet_frac) * Atrans,
        bkg_counts2,
        bkg_err2,
        sys_err=sys_err2,
    )
...
def min_det_fp_nllh(
    ...
    for i in range(Nsrcs):
        ps_rate_errs = np.array(
            [params[PSnames[i] + "_rate_" + str(j)] for j in range(nebins)]
        )
...
def min_det_fp_nllh2(
    ...
    bkg_diffAs = 1.0 - bkg_flatAs
    bkg_err0 = (
        bkg_rate_errs * (bkg_flatAs * Ndets0 + bkg_diffAs * solid_ang_dmean0) * dt
    )
    bkg_err1 = (
        bkg_rate_errs * (bkg_flatAs * Ndets1 + bkg_diffAs * solid_ang_dmean1) * dt
    )
    ...
    #     x0s = [[1., 1.], [2., 1.],
    #            [1., 2.], [2., 2.]]
    x0s = [[1.5, 0.725], [2.0, 1.105], [2.25, 1.605], [2.5, 1.995]]
...
class rates_fp_llh(object):
    def __init__(
        ...
        self.Atrans = np.array([0.05, 0.09, 0.125, 0.25, 0.4, 0.65])
    ...
    def get_cnts_tbins_ebins_fps(self, dur):
        ...
        h = np.histogramdd(
            [self.ev_data["TIME"], self.ev_data["ENERGY"]], bins=[tbins, ebins]
        )[0]
...
def main(args):
    ...
    for sim_tab_fname, sim_id in zip(sim_tab_fnames, simids):
        ...
        save_fname = os.path.join(
            args.sim_dir, "rates_llh_res_%d_%d.csv" % (args.job_id, sim_id)
        )
