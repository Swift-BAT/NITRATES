...
def cli():
    ...
    parser.add_argument("--Njobs", type=int, help="Total number of jobs", default=64)
    parser.add_argument("--jobid", type=int, help="Which job this is", default=-1)
    parser.add_argument(
...
def min_det_fp_nllh(
    data_counts, bkg_obj, t, dt, cnts_intp, Ndets, Ndets_total, get_bkg_nllh=False
):
    bkg_rate, bkg_rate_err = bkg_obj.get_rate(t)
    bkg_cnts = bkg_rate * dt * (Ndets / (float(Ndets_total)))
    bkg_err = bkg_rate_err * dt * (Ndets / (float(Ndets_total)))
...
def main(args):
    logging.basicConfig(
        filename="rates_llh_analysis_%d.log" % (args.jobid),
        level=logging.DEBUG,
