...
from ..archive.min_nlogl_from_seeds import llh_ebins_square,\
                                        get_abs_cor_rates
...
class llh_ebins_square_wsampler(object):
    ...
    def __init__(self, event_data, drm_obj, rt_obj,\
                 ebins0, ebins1, dmask, sys_errs, bkg_t0,\
                 bkg_dt, t0, dt, imx0, imx1,\
                 imy0, imy1, mk_bkg=False, E0=50.0):
        ...
        self.ind_ax = np.linspace(-2., 4., 20*6+1)
        ...
        self.rates_intp = get_cnts_intp_obj(self.ind_ax,\
                                        self.drm,\
                                        self.ebin_ind_edges,\
                                        self.abs_cor,\
                                        E0=E0, normed=False)
        ...
        self.bkg_rate_errs = .1*self.bkg_rates
...
#                                 6.*np.ones(self.nebins))
        ...
        lwrs = np.append([self.imx0, self.imy0, 1e-5,
                                 self.ind_ax[0]+.5],\
                                .25*np.ones(self.nebins))
        self.lowers = np.append(lwrs, np.ones(self.nebins))

        uprs = np.append([self.imx1, self.imy1, 5e2,
                                 self.ind_ax[-1]-.5],\
                                4.*np.ones(self.nebins))
    ...
    def flux2rate(self, A, ind):
        ...
        rate_ebins = np.array(get_cnt_ebins(A, ind, self.drm,\
                            self.ebin_ind_edges, E0=self.E0,\
                                    abs_cor=self.abs_cor))
    ...
    def like(self, imx, imy, A, ind, sig_rate_norms, bkg_rates):
        ...
        llh = np.sum(log_pois_prob(mod_cnts,\
                    self.data_cnts_blm))
    ...
    def calc_logprior(self, bkg_rates):
        ...
        logprior = stats.norm.logpdf(bkg_rates, loc=self.bkg_rates,\
                                           scale=self.bkg_rate_errs)
    ...
    def lnprior(self, theta):
        ...
        imx, imy, A, ind, sig_rate_norms, bkg_rates =\
                self.theta2params(theta)
...
#                             self.data_cnts_blm))
    ...
    def mk_sampler(self, nwalkers=None):
        ...
        self.sampler = emcee.EnsembleSampler(self.nwalkers, self.npars,\
                                        self.lnprob)
