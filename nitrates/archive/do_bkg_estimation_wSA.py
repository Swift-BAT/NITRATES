...
def main(args):
    ...
    logging.basicConfig(filename='bkg_rate_estimation_wSA.log', level=logging.DEBUG,\
    ...
    bkg_t0s = np.arange(-args.twind*1.024, args.twind*1.024 + 1, bkg_tstep) +\
                    trigtime - (bkg_dur + sig_wind)/2.

    llh_bkg = LLH_webins(ev_data, ebins0, ebins1, bl_dmask,\
                         t0=trigtime, t1=trigtime+bkg_dur, model=bkg_mod)
    ...
    for i in range(len(bkg_t0s)):
        ...
        if args.bkg_nopost:
            ...
            t0s = np.array([bkg_t0s[i]+bkg_dur/2.+sig_wind])
        else:
            t0s = np.array([bkg_t0s[i],bkg_t0s[i]+bkg_dur/2.+sig_wind])
        t1s = t0s + bkg_dur/2.
        # bkg_tax.append((t0s[0]+t1s[1])/2.)
        bkg_tax.append(bkg_t0s[i] + (bkg_dur + sig_wind)/2.)
        llh_bkg.set_time(t0s,t1s)

        bf_params = {}
        for e0 in range(nebins):
            bkg_miner.set_fixed_params(bkg_miner.param_names)
            e0_pnames = [pname for pname in bkg_miner.param_names if int(pname[-1])==e0]
    #         print e0_pnames
