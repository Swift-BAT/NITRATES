...
def analysis_at_theta_phi(theta, phi, bkg_bf_params, bkg_mod, flux_mod, ev_data,\
...
def main(args):
    ...
    for hp_ind in range(hp_ind0,hp_ind1):
        ...
        logging.info("At RA, Dec: %.3f, %.3f" %(ra, dec))
        if ang_sep(ra, dec, pnt_ra, pnt_dec) <= 40:
            logging.info("In FoV, skipping this pix")
            continue
        theta, phi = convert_radec2thetaphi(ra, dec, att_q)

        res_df = analysis_at_theta_phi(theta, phi, bkg_bf_params, bkg_mod, flux_mod,\
