...
from ..lib.funcs2run_bat_tools import std_grb, do_bkg, do_pc,\
                        get_sig_pix_mp, mk_sky_sig_img
...
def cli():
    ...
    parser.add_argument('--dbfname', type=str,\
            ...
            default=None)
    parser.add_argument('--TScut', type=float,\
            help="Min rate TS to make an image for",\
            default=2)
    ...
    parser.add_argument('--Nproc', type=int,\
            ...
            default=4)
    parser.add_argument('--os', type=int,\
            help="Oversampling",\
            default=2)
...
def main(args):
    ...
    dpi_bkg_fname = do_bkg(bkg_t0, bkg_t1, ev_fname,\
                        dmask, work_dir)
    logging.info("DPI bkg from MET %.3f - %.3f" %(bkg_t0, bkg_t1))

    timeIDs, RateTSvals = get_top_rate_timeIDs(conn, TScut=TScut,\
                                                ret_TSs=True)
    Nimgs = len(timeIDs)
    conn.close()
    ...
    get_sig_pix_mp(args.Nproc, times, dts, ev_fname, dpi_bkg_fname, pc_fname,\
                att_fname, dmask, work_dir, db_fname=db_fname,\
                timeIDs=timeIDs, RateTSs=RateTSvals, oversamp=args.os)
