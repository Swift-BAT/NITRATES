...
def do_init_bkg_wPSs(
    bkg_mod, llh_obj, src_tab, rt_obj, GTI, sig_twind, TSmin=8.0, Nprocs=1
):
        ...
        for name, TS in TS_nulls.items():
            ...
            if TS < TSmin_:
                ...
                if np.all(ps_rates < min_rate):
                    continue
            if np.all(ps_rates < (min_rate / 4.0)):
                continue
...
def main(args):
    ...
    if Nsrcs > 0:
        ...
        init_bf_params, src_tab = do_init_bkg_wPSs(
            bkg_mod, llh_obj, src_tab, rt_obj, GTI, sig_twind, Nprocs=Nprocs
        )
