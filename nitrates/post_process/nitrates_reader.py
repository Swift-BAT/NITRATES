...
def read_manager_log(path):
    ...
    try:
        with open(os.path.join(path, 'manager.log')) as fob:
            x = fob.read()
            startlocal = datetime.fromisoformat(re.search(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}),\d+-INFO- Wrote pid:",x)[1])
            start=tzlocal.localize(startlocal).astimezone(utc)
            #print(f'start:{start}')
            bkgstartlocal = datetime.fromisoformat(re.search(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}),\d+-INFO- Job submitted",x)[1])
            bkgstart = tzlocal.localize(bkgstartlocal).astimezone(utc)
            #print(f'bkg start: {bkgstart}')
            splitstartlocal = datetime.fromisoformat(re.search(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}),\d+-INFO- Jobs submitted",x)[1])
            splitstart = tzlocal.localize(splitstartlocal).astimezone(utc)
            #print(f'split start: {splitstart}')
            splitdonelocal = datetime.fromisoformat(re.search(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}),\d+-INFO-\s+Done with rates analysis",x)[1])
            ...
            submitIFOV = re.search(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}),\d+-INFO- Submitting (\d+) in FoV Jobs now", x)
            ...
            submitOFOV = re.search(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}),\d+-INFO- Submitting (\d+) out of FoV Jobs now", x)
            ...
            OFOVdonelocal = datetime.fromisoformat(re.search(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}),\d+-INFO- Got all of the out results now",x)[1])
            OFOVDone = tzlocal.localize(OFOVdonelocal).astimezone(utc)
            #print(f'OFOV done: {OFOVdone}')
            IFOVdonelocal = datetime.fromisoformat(re.search(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}),\d+-INFO- Got all of the in results now",x)[1])
            IFOVDone = tzlocal.localize(IFOVdonelocal).astimezone(utc)
            #print(f'IFOV done: {IFOVdone}')
    except Exception as e:
        print (e)
    
    return  start, bkgstart,splitstart,splitdone,Nsquares,Ntbins,Ntotseeds,OFOVfilesTot,IFOVfilesTot ,submitIFOVstamp, NjobsIFOV,submitOFOVstamp,NjobsOFOV,OFOVDone,IFOVDone, OFOVfilesDone, IFOVfilesDone
...
def grab_split_rate_results(path, files, trig_id, att_q, trigtime, top_n=64,notDB=False,config_id=0):
    ...
    dfs = [pd.read_csv(os.path.join(path, file)) for file in filter(split_rate_regex.fullmatch, files)]

    # Various sources of x-ray radiation I think? that pop up in split rate results CSVs when they are within bat FOV
    sources = ['1A 0535-262j', 'Sco X-1', 'GX 304-1', 'Vela X-1',
               'Cyg X-1', 'GX 301-2', '4U 1700-377', 'Crab Nebula',
               'EXO 2030+375', 'GRO J1008-57', 'Swift J1745.1-2624',
               'XTE J1752-223', 'GRO J1655-40', 'GX 339-4', 'GRS 1915+105',
               '1A 1118-61', 'Cyg X-3']
    ...
    src_renaming = {x + '_rt_sum': 'src_' + re.sub(r'[ \+\.-]', '_', x) + '_rt_sum' for x in sources}

    split_rate_results = pd.concat(dfs, axis=0, ignore_index=True).rename(mapper=src_renaming, axis=1)

    # Grabbing top_n results by test statistic
    split_rate_results = split_rate_results.iloc[split_rate_results['TS'].nlargest(top_n).index]
    ...
    ras,decs=convert_theta_phi2radec(split_rate_results.theta,split_rate_results.phi,att_q)
    ra_dec=[f'POINT({ra} {dec})' if not np.isnan(ra) else None for ra,dec in zip(ras,decs)] 
    split_rate_results['ra_dec'] = ra_dec

    return split_rate_results


def grab_out_fov_results(path, files, trig_id, att_q, trigtime, top_n=1000000, notDB=False, cluster=True, config_id=0):
    ...
    if cluster:
        dfs = [pd.read_csv(os.path.join(path, file), index_col=0) for file in filter(out_fov_regex.fullmatch, files)]
        ...
        subprocess.run(f'tail -q -n +2 res_hpind*.csv >> merged.csv',cwd=path,shell=True)
        subprocess.run('rm merged.csv', cwd=path,shell=True)
        out_fov_results = pd.read_csv(os.path.join(path, 'merged.csv'), index_col=None,engine='pyarrow')
    ...
    ras,decs=convert_theta_phi2radec(out_fov_results.theta,out_fov_results.phi,att_q)
    ra_dec=[f'POINT({ra} {dec})' for ra,dec in zip(ras,decs)] 
    out_fov_results['ra_dec'] = ra_dec
    phi_theta = [f'POINT({phi} {theta-90})' for theta,phi in zip(out_fov_results.theta,out_fov_results.phi)] 
    out_fov_results['phi_theta']= phi_theta

    return out_fov_results


def grab_in_fov_results(path, files, trig_id, att_q, trigtime, top_n=1000000, notDB=False, cluster=True, config_id=0):
    ...
    if cluster:
        dfs = [pd.read_csv(os.path.join(path, file), index_col=0) for file in filter(in_fov_regex.fullmatch, files)]
        ...
        subprocess.run(f'tail -q -n +2 res_*.csv >> resmerged.csv',cwd=path,shell=True) 

        in_fov_results_res = pd.read_csv(os.path.join(path, 'resmerged.csv'), index_col=None,engine='pyarrow')
        subprocess.run('rm resmerged.csv', cwd=path,shell=True)
        if peak:
            subprocess.run(f'sed -n 1p {filepeak} > peakmerged.csv',cwd=path,shell=True)
            subprocess.run(f'tail -q -n +2 peak_*.csv >> peakmerged.csv',cwd=path,shell=True) 
            in_fov_results_peak = pd.read_csv(os.path.join(path, 'peakmerged.csv'), index_col=None,engine='pyarrow')
            subprocess.run('rm peakmerged.csv', cwd=path,shell=True)
            in_fov_results = pd.concat([in_fov_results_res,in_fov_results_peak], axis=0, ignore_index=True)
        else:
    ...
    inds = in_fov_results.groupby(['squareID', 'timeID'])['TS'].transform(max) == in_fov_results['TS']
    ...
    phi_theta = [f'POINT({phi} {theta-90})' for theta,phi in zip(in_fov_results.theta,in_fov_results.phi)] 
...
def get_dlogls_inout(res_tab, res_out_tab,trigger_id,config_id=0,imdistthresh=0.012):
    ...
    tuples=list(zip(TSs,dlogls,dlogls_in_out,radecs,phithetas,As,Epeaks,gammas,timeIDs,dts,durs))
    df=pd.DataFrame(tuples,columns=['maxTS','DeltaLLHPeak','DeltaLLHOut','ra_dec','phi_theta','A','Epeak','gamma','timeID','dt','dur',])
    ...
    for i, path in enumerate(paths):
        try:
            ...
            # If not, create a new entry in the triggers table
                ...
                index = next((i for i, c in enumerate(dir_name) if c.isdigit()), len(dir_name))
                instrument_code= dir_name[:index]

                #instrument_code = re.sub(r'\d', '', dir_name)
                event_numbers = dir_name[index:]

                instrument = {'F': 'Fermi/GBM', 'C': 'CALET', 'IC': 'IceCube', 'I': 'INTEGRAL',
                            'FRB': 'CHIME', 'S': 'IGWN', 'H':'HAWC', 'Gutn':'GECAM'}.get(instrument_code)
                trigger_type = {'Fermi/GBM': 'GRB', 'IceCube': 'neutrino', 'INTEGRAL': 'GRB', 'CALET': 'GRB',
                                'CHIME': 'FRB', 'IGWN': 'GW', 'HAWC':'GRB','GECAM':'GRB'}.get(instrument)
                ...
                response = api.post_trig(trigger_time=datetimes[i].isoformat(),trigger_instrument=instrument,
                              trigger_name=f"{instrument} {event_numbers}",trigger_type=trigger_type)
            ...
            try:
                ...
                IFOVfilesTot=IFOVfilesTot, IFOVfilesDone=IFOVfilesDone)
                print('Posted log')
            except Exception as e:
            ...
            try:
                ...
                att_qs[i] = attfile['QPARAM'][np.argmin(np.abs(attfile['TIME'] - sctimes[i]))]
            except Exception as e:
