...
def cli():
    ...
    parser.add_argument(
        "--pas", type=str, help="password for email receiving LVC emails", default=None
    )
    parser.add_argument(
        "--imap_server",
        type=str,
...
def main(args):
    ...
    try:
        ...
        logging.warning(
            "Will try to get skymaps but won't try to update DB or run analysis"
        )
        email_error = True
    ...
    if db_exists:
        if np.sum(ID_bl) > 0:
            ...
            idx = (
                NewTrigger_df.groupby(["SID"])["far"].transform(min)
                == NewTrigger_df["far"]
            )
