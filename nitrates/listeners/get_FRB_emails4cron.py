...
def main(args):
    ...
    try:
        data_dicts = fetch_parse_FRB_emails(M, nums)
    except Exception as E:
        logging.error(E)
        logging.error(traceback.format_exc())
        logging.warning("Error fetching FRB emails")
        email_error = True
    ...
    if db_exists:
        ID_bl = ~np.isin(df_new["event_no"], df_old["event_no"])
    ...
    if db_exists:
        if np.sum(ID_bl) > 0:
            ...
            for index, row in NewTrigger_df.iterrows():
                new_trig_ids.append(row["event_no"])
                new_trig_times.append(row["time"])
                ras.append(row["ra"])
                decs.append(row["dec"])
                pos_errors.append(row["pos_error"])
        ...
        if len(df_new) > 0:
            New_Trigger = True
            new_trig_ids = df_new["event_no"]
            new_trig_times = df_new["time"]
            ras = df_new["ra"]
            decs = df_new["dec"]
            pos_errors = df_new["pos_error"]
            Nnewtrigs = len(new_trig_ids)
            logging.info(str(Nnewtrigs) + " new events")
    ...
    if args.run_analysis and New_Trigger:
        for i, trigID in enumerate(new_trig_ids):
            trid_id_str = "FRB" + str(int(trigID))
            ...
            sky_map = err_circle2prob_map(ras[i], decs[i], pos_errors[i], sys_err=1.0)
