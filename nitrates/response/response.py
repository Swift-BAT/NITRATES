...
class Response(object):
    ...
    def __init__(self, drm, ebins=None, cor_type='op'):
        ...
        self.drm_emids = ((drm[1].data['ENERG_LO'] +\
                          drm[1].data['ENERG_HI'])/2.).astype(np.float)
...
#         print self.nebins, "ebins"
    ...
    def set_abs_cor(self, cor_type='op'):
        ...
        if cor_type == 'op':
            self.abs_cor = 1./self.absorbs
        elif cor_type == 'trans':
            self.abs_cor = (2. - self.absorbs)/(self.absorbs)
...
class ResponseDPI(object):
    ...
    def set_pha_bins(self, pha_emins, pha_emaxs):
        ...
        if self.Nphabins == len(self.orig_pha_emins):
            if np.all(np.isclose(self.pha_emins, self.orig_pha_emins)) and\
                    np.all(np.isclose(self.pha_emaxs, self.orig_pha_emaxs)):
    ...
    def mk_resp_dpis(self):
        ...
        lines_resp_dpis, comp_resp_dpis = resp_tab2resp_dpis(self.resp_tab, phi_rot=self.phi0,\
                                        correct_comp_theta_gt110=self.correct_comp)
...
#           */
        ...
        passive_ind=np.power(np.power(psv[3]*np.power(E,psv[4]),
            1.0/psv[7])+np.power(psv[5]*np.power(E,
            psv[6]),1.0/psv[7]),psv[7])
...
#           */
        ...
        passive_ind=np.power(np.power(psv[9]*np.power(E,psv[10]),
            1.0/psv[13])+np.power(psv[11]*np.power(E,
            psv[12]),1.0/psv[13]),psv[13])
...
class Swift_Mask_Interactions(object):
    ...
    def __init__(self, rt_obj, bl_dmask, pass_trans=False):
        ...
        shrink_size = 0.1
        self.verts = np.array([(121.92-shrink_size, 60.96-shrink_size, 103.187),
...
#         trans_vec = mask_off_vec
    ...
    def set_energy_arr(self, energy):
        ...
        self.mask_tot_rho_mus_list = [mat.get_tot_rhomu(self.energy) for mat in self.mask_materials]
...
#         does_int_fix = np.zeros(self.ndets, dtype=np.bool)
    ...
    def get_trans(self):
        ...
        self.trans[self.does_int_mask] = self.mask_trans*self.fix_trans[self.does_int_mask]*\
...
#             rate_dpis[:,j] += np.sum(photon_fluxes*self.lines_resp_dpi[:,:,j],axis=1)
    ...
    def get_non_flor_rate_dpis_from_photon_fluxes(self, photon_fluxes):
        ...
        rate_dpis = get_rate_dpis_from_photon_fluxes(self.non_flor_resp_dpi, photon_fluxes)
#         rate_dpis = np.zeros((self.ndets,self.Nphabins))
#         for j in range(self.Nphabins):
...
def sand_imgs2dpis(sand_imgs):
    ...
    dpis = np.zeros((dpi_shape[0],dpi_shape[1],sand_imgs.shape[-2],sand_imgs.shape[-1]))
...
@njit(cache=True)
def shift_sand_img_pha_bins(sand_imgs, pha_bins0, pha_bins1, new_pha_bins0, new_pha_bins1):
    ...
    for k in range(NphotonEs):
        for i in range(16):
            for j in range(16):
                new_sand_imgs[k,i,j] += shift_pha_bins(sand_imgs[k,i,j], pha_bins0, pha_bins1,\
                                                    new_pha_bins0, new_pha_bins1)
...
#         self.batxs, self.batys = bldmask2batxys(self.bl_dmask)
        ...
        self.Nside = Nside
        all_hp_phis, all_hp_lats = hp.pix2ang(self.Nside, np.arange(hp.nside2npix(self.Nside),\
                                                                    dtype=np.int), lonlat=True)
        all_hp_thetas = 90.0 - all_hp_lats
        self.max_hp_theta = np.max(all_hp_thetas)
        self.min_hp_theta = np.min(all_hp_thetas)
        self.min_hp_theta = 60.0 # adjust this when the next theta set of flor resps are made
        self.hp_phis_at_max_theta = np.sort(all_hp_phis[(np.isclose(all_hp_thetas,self.max_hp_theta))])
        self.hp_phis_at_min_theta = np.sort(all_hp_phis[(np.isclose(all_hp_thetas,self.min_hp_theta))])
    ...
    def do_Eintp(self, resp_sand_imgs):
        ...
        new_resp_sand_imgs = resp_sand_imgs[self.Einds0]*self.E_A0s +\
    ...
    def set_theta_phi(self, theta, phi):
        ...
        if self.theta > self.max_hp_theta:
            ...
            self.hp_inds2use = [hp.ang2pix(self.Nside, phi0, 90.0-self.max_hp_theta, lonlat=True),
            ...
            self.hp_inds2use = [hp.ang2pix(self.Nside, phi0, 90.0-self.min_hp_theta, lonlat=True),
    ...
    def open_new_file(self, hp_ind, fname=None):
        ...
        if self.ShiftEbins:
            resp_sand_imgs0 = shift_sand_img_pha_bins(resp_sand_imgs0, self.orig_pha_emins,\
                                              self.orig_pha_emaxs, self.pha_emins, self.pha_emaxs)

        self.resp_dict[hp_ind] = np.swapaxes(np.swapaxes(self.do_Eintp(resp_sand_imgs0),0,2),0,1)
...
#         self.resp_dpi_shape = (173, 286, self.NphotonEs, self.Nphabins)
    ...
    def setup_comp_flor_obj(self):
        ...
        if self.use_comp_flor4flor_only:
            self.comp_flor_resp_obj = CompFlorResponseDPI(self.flor_resp_dname2, self.bl_dmask)
        else:
            self.comp_flor_resp_obj = CompFlorResponseDPI(self.comp_flor_resp_dname, self.bl_dmask)
        self.comp_flor_resp_obj.set_photonEs(self.PhotonEs)
        self.comp_flor_resp_obj.set_ebins(self.pha_emins, self.pha_emaxs)
...
#             rate_dpis[:,j] += np.sum(photon_fluxes*self.comp_resp_dpi[:,:,j],axis=1)
    ...
    def get_comp_flor_rate_dpis_from_photon_fluxes(self, photon_fluxes):
        ...
        rate_dpis = get_rate_dpis_from_photon_fluxes(self.comp_flor_resp_dpis, photon_fluxes)
        return rate_dpis
