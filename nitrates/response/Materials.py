...
class Element(object):
    ...
    def __init__(self, name, cross_section_dname=None):
        ...
        self.tot_mu_intp = interpolate.interp1d(np.log10(self.energies),\
                                                np.log10(self.tab['TotwCoh']))
        self.photoe_mu_intp = interpolate.interp1d(np.log10(self.energies),\
                                                np.log10(self.tab['PhotoelAbsorb']))
        self.comp_mu_intp = interpolate.interp1d(np.log10(self.energies),\
                                                np.log10(self.tab['IncoherScatter']))

    def get_tot_rho(self, Energy):
        return 10.**self.tot_mu_intp(np.log10(Energy))

    def get_photoe_rho(self, Energy):
        return 10.**self.photoe_mu_intp(np.log10(Energy))

    def get_comp_rho(self, Energy):
        return 10.**self.comp_mu_intp(np.log10(Energy))
...
class Material(object):
    ...
    def get_tot_rhomu(self, Energy):
        ...
        for element in self.elements:
            tot_rhomu += self.mass_frac_dict[element]*self.density*\
    ...
    def get_comp_rhomu(self, Energy):
        ...
        for element in self.elements:
            rhomu += self.mass_frac_dict[element]*self.density*\
