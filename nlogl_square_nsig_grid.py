...
def nllhs_data_cube(data, sig_mod, bkg_model, Nsigs):
    ...
    for i, Nsig in enumerate(Nsigs):
        ...
        nllhs[i] = -1.*np.sum(log_pois_prob(model, data),\
                                 axis=(1,2))
...
class llh_ebins(object):
    ...
    def __init__(self, event_data, drm_obj, rt_obj,\
        ...
        self.set_ray_trace()
        self.drm, self.drm_imx, self.drm_imy =\
            self.drm_obj.get_drm(imx, imy, ret_pos=True)
        self.ebin_ind_edges = get_ebin_ind_edges(self.drm,\
                                        self.ebins0, self.ebins1)

        self.sig_ebins_normed_perind = [get_cnt_ebins_normed(ind,\
                                    self.drm, self.ebin_ind_edges)\
                                    for ind in self.inds]


        self.lowers = [-1., -.5, .2, .2, .2, .2]
        self.uppers = [4., 2.5, 10., 10., 10., 10.]
    ...
    def set_bkg_time(self, t0, dt):
        ...
        self.bkg_off_cnts = np.array([(self.bkg_cnts[i]/self.ndets)*np.ones(self.ndets) for\
        ...
        self.bkg_errors = .1*self.bkg_off_cnts
    ...
    def update_imxy(self, imx, imy):
        ...
        self.imx = imx
        self.imy = imy
        if np.hypot((imx-self.drm_imx), (imy-self.drm_imy)) > .1:
            self.drm, self.drm_imx, self.drm_imy =\
                self.drm_obj.get_drm(imx, imy, ret_pos=True)
            self.sig_ebins_normed_perind = [get_cnt_ebins_normed(ind,\
                                        self.drm, self.ebin_ind_edges)\
    ...
    def get_cube_nllhs(self, Nsig_ax):
        ...
        for i in xrange(len(self.inds)):
            nllhs_perind.append(nllhs_data_cube(self.data_cube,\
                    self.sig_mods[i], self.bkg_model, Nsig_ax))
    ...
    def signal_model(self, imx, imy, sig_cnts, index_ind):
        ...
        mod_cnts = np.array([rt_bl*sig_cnts_per_ebin[i] for\
    ...
    def unnorm_params(self, theta):
        ...
        sig_cnts = 10.**( theta[0]*(self.uppers[0] - self.lowers[0]) + self.lowers[0] )
        if len(theta) < 2:
            return sig_cnts
    ...
    def nllh_profiled_bkg(self, theta):
        ...
        sig_model_cnts = self.signal_model(self.imx, self.imy,\
                                           sig_cnts, index)


        nllh = -1.*np.sum(profile_pois_logl(self.data_cnts_blm,\
                                            self.bkg_off_cnts,\
                                            self.bkg_dt, self.sig_dt,\
                                            self.bkg_errors,\
                                            sig_model_cnts))
    ...
    def nllh_profiled_bkg_fixind(self, theta, index_ind):
        ...
        nllh = -1.*np.sum(profile_pois_logl(self.data_cnts_blm,\
                                            self.bkg_off_cnts,\
                                    self.bkg_dt, self.sig_dt,\
                                    self.bkg_errors, sig_model_cnts))

        return nllh



    def min_nllh(self, meth='L-BFGS-B', x0=None, maxiter=100,\
                 seed=None, fix_ind=None):
        ...
        if meth == 'dual_annealing':
            ...
            if fix_ind is not None:
                ...
                res = optimize.minimize(func2min, x0, method=meth,\
                                        args=args,\
                                        bounds=bnds, tol=.1)
            else:
                res = optimize.minimize(func2min, x0, method=meth,\
...
def main(args):
    ...
    llh_obj = llh_ebins(ev_data, drm_obj, rt_obj, ebins0,\
                            ebins1, dmask, bkg_t0, bkg_dt,\
                            sig_t0, sig_dt, args.imx0,\
                            args.imy0, ind_ax)


    names = ['exp', 'time', 'bkg_nllh',
            'sig_nllh', 'imx', 'imy', 'sig_cnts', 'index']
    dtypes = [np.float for i in xrange(len(names))]
    ...
    for ii in xrange(len(test_dts)):
        ...
        for jj in xrange(len(sig_t0_ax)):
            ...
            bkg_nllh = llh_obj.nllh_profiled_bkg_fixind([0.], 0)
            print bkg_nllh
            bkg_nllhs.append(bkg_nllh)

            for kk in xrange(npnts):
