...
def cli():
    ...
    parser.add_argument('--Nside', type=int,\
            help="Nside for mosaiced healpix map",\
            default=2**10)
    parser.add_argument('--t0', type=float,\
            help="Min image start time to use (MET (s))",\
            default=0.0)
    parser.add_argument('--dt', type=float,\
            help="Duration in secs to use, picks images from t0 to t0+dt",\
            default=2.5)
    parser.add_argument('--pc_fname', type=str,\
            help="partial coding file name",\
            default='pc_4.img')
    parser.add_argument('--dname', type=str,\
            help="Directory to look for images",\
            default='.')
...
def get_sky_and_bkgvar_fnames(dname, t0, t1):
    ...
    dtp = [('sky_fname',sky_fnames.dtype), ('t0',np.float), ('t1',np.float),
...
def get_and_add_skymaps(Nside, dname, fname_arr, pc_fname, pc_min=0.01):
    ...
    for i in range(Nimgs):
        print("opening files")
        sky_file = fits.open(os.path.join(dname,fname_arr[i]['sky_fname']), lazy_load_hdus=False)
...
class Worker(mp.Process):
    ...
    def __init__(self, result_queue, inds2do, hp_map_vecs, sky_map, sig_rad=0.5, bkg_rad=2.5):
...
def sky_map2bkg_maps_mp(sky_map, pc_map, pc_min=1e-1, sig_rad=0.5, bkg_rad=2.5, Nprocs=4):
    '''
...
def main(args):
    ...
    summed_sky_map, Nmaps, pc_tot = get_and_add_skymaps(args.Nside, args.dname,\
                                                    fname_arr, args.pc_fname)


    # make bkg and bkg_var/noise maps
    if args.Nproc > 1:
        bkg_map, bkgvar_map = sky_map2bkg_maps_mp(summed_sky_map, pc_tot, Nprocs=args.Nproc)
    else:
