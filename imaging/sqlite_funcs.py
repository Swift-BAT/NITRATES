...
from time_funcs import utc2met, met2astropy, met2utc_str, met2mjd
...
def get_sql_tab_list():
    ...
    sql_tab_list = [sql_tab_info, sql_tab_quads, sql_tab_twinds, sql_tab_ratefits,
                    sql_tab_rates, sql_tab_blips, sql_tab_img,
                    sql_tab_res, sql_tab_files, sql_tab_seeds,
                    sql_tab_twind_stat, sql_tab_sigimg, sql_tab_job_square,
                    sql_tab_square_stat, sql_tab_square_res]
...
def write_JobSquare_line(conn, data_dict):
    ...
    sql = '''INSERT INTO JobSquare (squareID, proc_group, imx0, imx1, imy0, imy1)
            VALUES (?, ?, ?, ?, ?, ?);'''
    conn.execute(sql, (data_dict['squareID'], data_dict['proc_group'],\
                        data_dict['imx0'], data_dict['imx1'],\
                        data_dict['imy0'], data_dict['imy1']))
    conn.commit()
...
def write_square_res_line(conn, data_dict):
    ...
    sql = 'INSERT INTO SquareResults (' + ','.join(col_names) +') values(' + ','.join(['?'] * len(values)) + ')'
...
def write_twind_line(conn, timeID, data_dict):
    ...
    sql = 'INSERT INTO TimeWindows (' + ','.join(col_names) +') values(' + ','.join(['?'] * len(values)) + ')'

    conn.execute(sql, values)
    conn.commit()


def setup_tab_twinds(conn, trig_time, ntdbls=4,\
                    min_bin_size=0.256, att_tab=None,\
                    acs_tab=None, acs_att_dt_max=30.0,\
                    t_wind=15, tmin=None, tmax=None, GTI=None):
...
def setup_files_tab(conn, evfname, att_fname, detmask, rt_dir, drm_dir, work_dir, bat_ml_dir):

    sql = '''INSERT INTO Files
            (evfname, detmask, attfname, drmDir, rtDir, workDir, batmlDir)
            VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s");'''\
            %(evfname, detmask, att_fname, drm_dir, rt_dir, work_dir, bat_ml_dir)
    conn.execute(sql)
    conn.commit()
...
def setup_tab_info(conn, ev_fname, trigtime):
    ...
    sql = '''INSERT INTO DataInfo
            ...
            VALUES (%f, %f, %f, "%s", "%s", "%s");'''\
            %(tmin, tmax, trigtimeMET, tmin_utc, tmax_utc, trigtimeUTC)
    conn.execute(sql)
...
def write_rate_fit_line(conn, rate_obj, ind, ebin, quadID, rate, err, chi2):
    ...
    sql = '''INSERT INTO RateFits
            ...
            (%f, %d, %d, %d, %d, %d, %f, %f, %f);
            '''\
            %(rate_obj.t_poly_ax[ind], quadID, 1,\
            int(rate_obj.post), ebin, rate_obj.deg,\
            rate, err, chi2)
    ...
    for i in xrange(rate_obj.n_lin_pnts):
        ...
        for j in xrange(rate_obj.nebins):
            ...
            if ~np.isfinite(errs[j]):
                errs[j] = 0.0
            write_rate_fit_line(conn, rate_obj, i, j, quadID, rates[j],\
            errs[j], chi2s[j])
...
def append_rate_tab(conn, df_twind, quadID, bkg_nllh, sig_nllh, bf_nsigs, bf_inds):
    ...
    TS[(np.isnan(TS))] = 0.0
    df_dict = {'timeID': df_twind.timeID, 'quadID': quadID,
                'time': df_twind.time, 'duration':df_twind.duration,
                'bkg_nllh': bkg_nllh, 'sig_nllh': sig_nllh,
                'TS': TS, 'nsig': bf_nsigs, 'PlawInd': bf_inds}
...
def update_twind_stat(conn, timeID, col='BlipsFounds', val=1):
    ...
    sql = "update TimeWindowStatus set %s=%d where timeID=%d"\
            %(col, val, timeID)
...
def write_cat2db(conn, cat_fname, timeID):
    ...
    df_dict = {'blipID':blipIDs, 'timeID': timeID,
                'time': cat['TIME'], 'duration':cat['EXPOSURE'],
                'snr': cat['SNR'], 'imx': cat['IMX'],
                'imy': cat['IMY'], 'pc': cat['PCODEFR']}
    df = pd.DataFrame(df_dict)
...
def write_seed_line(conn, blipID, timeID, time, duration,\
                    imx, imy, proc_group, priority):
...
def write_seeds(conn, blip_df):
    ...
    for i in xrange(len(blip_df)):
        ...
        write_seed_line(conn, blip_df['blipID'][i], blip_df['timeID'][i],\
                        ...
                        blip_df['imx'][i], blip_df['imy'][i],\
                        blip_df['proc_group'][i], blip_df['priority'][i])

    update_twind_stat(conn, blip_df['timeID'][0], col='SeedsAssigned')


def write_result(conn, seed_tab_row, sig_param_dict,\
...
#            (?, ?, ?, ?, ?, ?, ?);'''
    ...
    sql = "update Seeds set done=1 where blipID=%d AND timeID=%d"\
            %(seed_tab_row.blipID, seed_tab_row.timeID)
...
def write_results_fromSigImg(conn, seed_tab_rows, t0s, dts, sig_param_dicts,\
                bkg_nllhs, sig_nllhs, TSs):
    ...
    for i, row in enumerate(seed_tab_rows):
        ...
        data.append((row['timeID'], t0s[i], dts[i],\
                    TSs[i], bkg_nllhs[i], sig_nllhs[i],\
                    imx, imy, A, ind))
